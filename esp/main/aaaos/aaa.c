...
void app_main()
{
    ...
    for (uint32_t idx = 0; idx < TaskTableLen; idx++)
    {
        ...
        SYS_ASSERT(pdFAIL != xTaskCreate(TaskList[idx].tFunc,
                                         TaskList[idx].tDesc,
                                         TaskList[idx].tDepth,
                                         &TaskList[idx].tId,
        ...
        AAA_LOGD(TAG, "CreateTask desc %s, id: %d, depth: %d, prio: %d, entry: %p",
                 TaskList[idx].tDesc,
                 TaskList[idx].tId,
                 TaskList[idx].tDepth,
                 TaskList[idx].tPriority,
...
{
    AAA_LOGD(TAG, "TaskPost id:%d, sig:%d, msg:%p, len:%d",
             tId,
             sig,
             msg,
             len);
    ...
    if (len)
    {
        ...
        AAA_LOGD(TAG, "TaskPost copy msg %p -> %p, len:%d",
                 msg,
                 aaaMsg.data,
                 len);
    }
...
bool AAATaskPostMsgFromISR(uint32_t tId, uint32_t sig, void *msg, uint32_t len)
{
    ...
    if (pdTRUE != xQueueSendFromISR(TaskList[tId].qHandle,
                                    &aaaMsg,
...
bool AAATaskRecvMsg(uint32_t tId, uint32_t *sig, void **msg, uint32_t *len)
{
    ...
    SYS_ASSERT(pdFAIL != xQueueReceive(TaskList[tId].qHandle,
                                       &aaaMsg,
                                       portMAX_DELAY));
...
static void prvAAATimerCallback(TimerHandle_t timHandler)
{
    ...
    AAATaskPostMsg(aaaTimArg->tId, aaaTimArg->sig, aaaTimArg->data, aaaTimArg->len);
    if (!aaaTimArg->reload)
    {
...
AAATimer_t AAATimerSet(uint32_t tId, uint32_t sig, void *msg, uint32_t len, uint32_t period, bool reload)
{
    AAA_LOGD(TAG, "TimerSet id:%d, sig:%d, msg:%p, len:%d, period:%u, reload:%s",
             tId,
             sig,
             msg,
             len,
    ...
    if (aaaTimArg->len)
    {
        ...
        AAA_LOGD(TAG, "TimerSet copy msg %p -> %p, len:%d",
                 msg,
                 aaaTimArg->data,
                 aaaTimArg->len);
    }

    TimerHandle_t timHandler = xTimerCreate("AAATim",
                                            pdMS_TO_TICKS(period),
                                            aaaTimArg->reload ? pdTRUE : pdFALSE,
                                            (void *)aaaTimArg,
                                            prvAAATimerCallback);
    if (timHandler != NULL)
    {
...
void AAATimerRemove(AAATimer_t timer)
{
    ...
    AAATimerArg_t *aaaTimArg = (AAATimerArg_t *)pvTimerGetTimerID(timHandler);
    AAA_LOGD(TAG, "TimerRemove: %p, id:%d, sig:%d",
             timer,
             aaaTimArg->tId,
             aaaTimArg->sig);
